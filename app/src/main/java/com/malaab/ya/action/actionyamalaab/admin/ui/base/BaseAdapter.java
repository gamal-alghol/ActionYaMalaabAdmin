...
//                    AppLogger.w("loading = " + loading + "\n totalItemCount = " + totalItemCount + "\n lastVisibleItem = " + lastVisibleItem);
    ...
    public void setItem(T item, int pos) {
        list.set(pos, item);
        this.notifyDataSetChanged();
    }
        ...
        if (originalList != null && originalList.size() > 0) {
            for (T item : originalList) {
                if (item instanceof User &&
                        (StringUtils.contain(((User) item).getUserFullName(), constraint)
                                || StringUtils.contain(((User) item).appUserId, constraint)
                                || StringUtils.contain(((User) item).mobileNo, constraint))) {
                    results.add(item);
                }
